#!/usr/bin/env python3
"""Standalone script to reprocess all files from GridFS into ChromaDB."""
...
async def reprocess_all_files():
    ...
    rag = RAGService()
    print(f"RAG service initialized. Collection: {rag.collection.name if rag.collection else 'None'}")

    # Get all file metadata
    files = await db.file_metadata.find({}).to_list(length=1000)

    print(f"\nFound {len(files)} files to reprocess from GridFS")
    ...
    for file_doc in files:
        ...
        try:
            ...
            print(f"  Downloaded {len(content)} bytes to: {temp_path}")
